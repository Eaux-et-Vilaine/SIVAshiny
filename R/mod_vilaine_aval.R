  ...
  tabPanel(title="Vilaine aval",
      ...
      sidebarLayout(
          ...
          mainPanel(width = 8,                       
              ...
              h3("Débits"),
              shinycssloaders::withSpinner(rAmCharts::amChartsOutput(ns("va_armchart_debits"))),
...
#              ZoomButton <- data.frame(Unit = c("mm", "mm", 'hh', 'DD',  'MAX'), multiple = c(1, 3 ,1 ,1, 1),
#                  label = c("10 min","30 min", "heure","jour", "MAX"))
        ...
        output$va_armchart_debits <- renderAmCharts({
              ...
              rAmCharts::amTimeSeries(
                      inner_join(va$debits$Q10,va$debits, 
                      'horodate',
                      colnames(va$niveaux)[2:ncol(va$niveaux)],
                      bullet = "round",
                      color =   randomcoloR::distinctColorPalette(ncol(va$niveaux)-1),
